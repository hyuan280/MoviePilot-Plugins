...
- 配置：
  ...
  - 无效剧名：对于一些没有意义的剧名，可以配置后使用父目录名称进行刮削
...
- 配置：
  ...
  - 下载器：MP已配置的下载器
  - 运行方式: 监控目录 还是 定时运行
  - 监控方式：选择监控目录时生效，可选 性能模式 和 兼容模式
  - 执行周期：选择定时运行时生效，cron表达式
  - 目录配置: 配置在哪些目录查找种子，和种子对应的保存路径
    - 1、一行一个，格式 "种子文件路径:保存路径"
    - 2、种子文件路径是mp容器的路径，保存路径为下载器保存路径
    - 3、一行开头写"#"代表注释这一行的配置
    - 4、不递归查找子目录，上传失败的种子会移动到failed目录下，成功的会移动到success目录下
