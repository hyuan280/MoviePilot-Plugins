...
class MTeamHandler(_ISiteHandler):
    ...
    """
    # 站点类型标识
    site_schema = SiteSchema.MTorrent
    ...
    def parse_invite_page(self, site_info: Dict[str, Any], session: requests.Session) -> Dict[str, Any]:
        ...
        # 记录站点配置信息（隐藏敏感内容）
        logger.info(f"站点 {site_name} 配置信息: URL={site_url}, API Key设置={bool(api_key)}")
        ...
        try:
            ...
            if not api_key:
                ...
                result["invite_status"]["reason"] = "API认证信息不完整，请在站点设置中配置API Key"
                return result

            # 提取API域名
            api_domain = StringUtils.get_url_domain(site_url)
