...
class ModuleLoader:
    ...
    def load_site_handlers() -> List[Type[_ISiteHandler]]:
        ...
        for filename in os.listdir(sites_dir):
            ...
            try:
                # 动态导入模块
                module = importlib.import_module(f"plugins.invitemanage.sites.{module_name}")
    ...
    @staticmethod
    def get_handler_for_site(site_schema: str, handlers: List[Type[_ISiteHandler]]) -> _ISiteHandler:
        """
        获取匹配站点的处理器实例
        ...
        for handler_class in handlers:
            if handler_class.site_schema.value == site_schema:
