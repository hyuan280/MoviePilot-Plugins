...
from plugins.invitemanage.data import DataManager
from plugins.invitemanage.utils import NotificationHelper, SiteHelper
from plugins.invitemanage.module_loader import ModuleLoader
...
class InviteManage(_PluginBase):
    # 插件名称
    plugin_name = "邀请管理系统"
    # 插件描述
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "invitemanage_"
    # 加载顺序
    plugin_order = 21
    # 可使用的用户级别
    auth_level = 2
                ...
                for site_id in config.get("site_ids", []):
                    ...
                    try:
                        if isinstance(site_id, str) and site_id.isdigit():
                            self._manage_sites.append(int(site_id))
                        elif isinstance(site_id, int):
                            self._manage_sites.append(site_id)
                    except:
                        pass
            # 保存配置
    ...
    def __reload_modules(self):
        ...
        try:
            ...
            for module_name in list(sys.modules.keys()):
                if module_name.startswith('plugins.invitemanage.') and module_name != 'plugins.invitemanage':
            ...
            importlib.import_module('plugins.invitemanage.data')
            importlib.import_module('plugins.invitemanage.utils')
            importlib.import_module('plugins.invitemanage.module_loader')
            ...
            try:
                from plugins.invitemanage.data import DataManager
                from plugins.invitemanage.utils import NotificationHelper
                from plugins.invitemanage.module_loader import ModuleLoader
    ...
    def _get_site_invite_data(self, site_name):
        ...
        try:
            ...
            # 先验证此站点是否在用户选择的站点列表中
            if self._manage_sites and str(site_id) not in [str(x) for x in self._manage_sites]:
    ...
    def refresh_all_sites(self) -> Dict[str, int]:
        ...
        try:
            ...
            # 筛选站点配置 - 如果_manage_sites为空，则选择所有站点
            selected_sites = []
            if not self._manage_sites:
                logger.info("未选择任何站点，将使用所有站点")
                ...
                for site in all_sites:
                    ...
                    manage_sites_str = [str(x) for x in self._manage_sites]

                    # 调试输出当前站点ID
                    logger.debug(f"检查站点ID: {site_id}，类型: {type(site_id)}")

                    if site_id_str in manage_sites_str:
            ...
            if selected_sites:
                ...
                logger.debug(f"选择的站点ID: {self._manage_sites}")
    ...
    def get_api_handlers():
        ...
        """
        return {
            "/get_invitees": {"func": invitemanage.get_invitees, "methods": ["GET"], "desc": "获取所有站点邀请数据"},
            "/refresh": {"func": invitemanage.refresh_data, "methods": ["GET"], "desc": "强制刷新站点数据"}
        }
    ...
    def update_config(self, request: dict) -> Response:
        ...
        try:
            ...
            # 记录站点ID，用于调试
            logger.info(f"已选择站点ID: {self._manage_sites}")
