...
class FilterSiteTorrent(_PluginBase):
    # 插件名称
    plugin_name = "筛选站点转移做种"
    # 插件描述
    plugin_desc = "从其他下载器中选出特定站点的种子到另一个下载器"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "filtersitetorrent_"
    # 加载顺序
    plugin_order = 18
    # 可使用的用户级别
    auth_level = 2
    ...
    _includetracker = None
    _notracker = None
    _deletesource = False
    _deleteduplicate = False
    _fromtorrentpaths = None
    _autostart = False
    _skipverify = False
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._includetracker = config.get("includetracker")
            self._notracker = config.get("notracker")
            self._frompath = config.get("frompath")
            self._topath = config.get("topath")
            self._fromdownloaders = config.get("fromdownloaders")
            ...
            self._fromtorrentpaths = config.get("fromtorrentpaths")
    ...
    def get_state(self):
        return True if self._enabled \
                       and self._cron \
                       and self._fromdownloaders \
                       and self._todownloader \
                       and self._fromtorrentpaths else False
                                ...
                                'content': [
                                    {
                                        'component': 'VTextarea',
                                        'props': {
                                            'model': 'includetracker',
                                            'label': '转移这些tracker的种子',
                                            'rows': 2,
                                            'placeholder': '每一行一个tracker域名'
                                        }
                                    }
                                ]
                            },
    ...
    def __validate_config(self) -> bool:
        ...
        """
        # 检查配置
        if not self._fromtorrentpaths:
            logger.error(f"源下载器种子文件未配置")
            self.systemmessage.put(f"源下载器种子文件未配置", title=self.plugin_name)
            return False

        from_services = [self.service_info(fromdownloader) for fromdownloader in self._fromdownloaders]
        fromtorrentpaths = self._fromtorrentpaths.split('\n')
        for from_service in from_services:
            for fromtorrentpath in fromtorrentpaths:
                fromtorrentconfig = fromtorrentpath.split('::')
                if len(fromtorrentconfig) != 2:
                    logger.warn(f"源下载器种子文件配置错误：{fromtorrentpath}")
                    continue
                if from_service.name == fromtorrentconfig[0]:
                    if Path(fromtorrentconfig[1]).exists():
                        self._fromtorrentconfig[from_service.name] = fromtorrentconfig[1]
                    else:
                        logger.error(f"源下载器种子文件保存路径不存在：{fromtorrentconfig[1]}")
                        self.systemmessage.put(f"源下载器种子文件保存路径不存在：{fromtorrentconfig[1]}", title=self.plugin_name)
                        return False
                    break

        if self._todownloader in self._fromdownloaders:
            logger.error(f"源下载器和目的下载器不能相同")
            self.systemmessage.put(f"源下载器和目的下载器不能相同", title=self.plugin_name)
            return False
        return True

    def __download(self, from_service: ServiceInfo, to_service: ServiceInfo, content: bytes,
    ...
    def transfer(self):
        ...
        for from_service in from_services:
            fromtorrentpath = self._fromtorrentconfig.get(from_service.name)
            ...
            for torrent in torrents:
                ...
                if self._includecategory:
                    ...
                    if torrent_category not in self._includecategory.split(','):
                        ...
                        continue
                # 根据设置决定是否转移无标签的种子
                if not is_torrent_labels_empty:
                    # 排除含有不转移的标签
                    if self._nolabels:
                        is_skip = False
            ...
            if trans_torrents:
                ...
                for torrent_item in trans_torrents:
                    # 检查种子文件是否存在
                    torrent_file = Path(fromtorrentpath) / f"{torrent_item.get('hash')}.torrent"
                    ...
                    if downloader_helper.is_downloader("qbittorrent", service=from_service):
                        if not main_announce:
                            ...
                            # 读取fastresume文件
                            fastresume_file = Path(fromtorrentpath) / f"{torrent_item.get('hash')}.fastresume"
                    ...
                    # 处理Tracker
                    if self._includetracker or self._notracker:
                        main_announce = torrent_main.get('announce')
                        if not main_announce:
                            logger.warn(f"种子文件 {torrent_file} 没有 Tracker， 失败")
                            fail += 1
                            continue
                        _,tracker_domain = StringUtils.get_url_netloc(main_announce)
                        # 选定Tracker
                        if not tracker_domain in self._includetracker.split('\n'):
                            logger.debug(f"种子文件 {torrent_file} 被过滤 {self._includetracker}， 跳过")
                            skip += 1
                            continue
                        # 排除Tracker
                        if tracker_domain in self._notracker.split('\n'):
                            logger.debug(f"种子文件 {torrent_file} 被排除 {self._notracker}， 跳过")
                            skip += 1
                            continue

                    # 发送到另一个下载器中下载：默认暂停、传输下载路径、关闭自动管理模式
                    logger.info(f"添加转移做种任务到下载器 {to_service.name}：{torrent_file}")
                    download_id = self.__download(from_service=from_service, to_service=to_service,
                                                content=torrent_file.read_bytes(),
